...
# MODEL_DIR = '/content/drive/My Drive/Colab Notebooks/bond_prediction_model' # Example: full path in Drive
...
# Define all possible categories for OHE features based on your trained_feature_names
# This is crucial for `encoder.transform` to work correctly when `handle_unknown='ignore'`.
# The encoder will learn these from the training data, but for new unseen categories, it will output zeros.
# For consistent UI inputs, we list the most common ones.
ALL_BOND_SECTORS = [
    'AUTO', 'COMM', 'CONS', 'ENERG', 'FIN', 'FOOD', 'PAPER',
    'PROP', 'STEEL', 'TOURISM', 'TRANS', 'Unknown' # Added 'Unknown' for robustness in case of unseen data
]
ALL_BOND_ISSUER_TYPES = ['GBS', 'Corporate', 'Government', 'Municipal', 'Other', 'Unknown'] # Expanded to match common simulation
ALL_BOND_GRADE_TYPES = ['Investment_Grade', 'Non_Investment_Grade', 'Unknown'] # Added 'Unknown'
ALL_BOND_SECURITY_TYPES = ['Secured', 'Unsecured', 'Convertible', 'Unknown'] # Added 'Unknown' and 'Convertible'
ALL_BOND_CREDIT_RATINGS = ['0', 'A_', 'B', 'BB', 'BB+', 'BB_', 'BBB', 'BBB+', 'BBB_', 'Non_rated',
                           'AAA', 'AA+', 'AA', 'AA-', 'A+', 'A', 'A-', 'CCC+', 'CCC', 'CCC-', 'CC', 'C', 'D'] # Comprehensive list
...
def load_model_and_encoder():
    try:
        ...
        # IMPORTANT: Define the feature names the model was trained on AFTER OHE.
        # This list MUST match the columns of X_processed in your training script.
        # This list is taken DIRECTLY from your provided list of features.
        trained_feature_names = [
            ...
            'bond_credit_rating_0', 'bond_credit_rating_A_', 'bond_credit_rating_B',
            'bond_credit_rating_BB', 'bond_credit_rating_BB+', 'bond_credit_rating_BB_',
            'bond_credit_rating_BBB', 'bond_credit_rating_BBB+', 'bond_credit_rating_BBB_',
            'bond_credit_rating_Non_rated', 'bond_sector_AUTO', 'bond_sector_COMM',
            'bond_sector_CONS', 'bond_sector_ENERG', 'bond_sector_FIN', 'bond_sector_FOOD',
            'bond_sector_PAPER', 'bond_sector_PROP', 'bond_sector_STEEL',
            'bond_sector_TOURISM', 'bond_sector_TRANS', 'bond_security_type_Secured',
            'bond_security_type_Unsecured', 'bond_grade_type_Investment_Grade',
            'bond_grade_type_Non_Investment_Grade', 'bond_issuer_type_GBS'
        ]
        st.info("Feature names for model loaded (VERIFIED with your provided list).")
        ...
        st.info("If your files are in the same directory as this script, ensure MODEL_DIR = '' or MODEL_DIR = '.'")
...
# --- Feature Preparation Function (for raw input before OHE and alignment) ---
def prepare_raw_input_data(df_to_prepare):
    ...
    for col in categorical_cols_raw:
        if col not in df_copy.columns:
            ...
            if col == 'bond_credit_rating': df_copy[col] = 'Non_rated'
            elif col == 'bond_issuer_type': df_copy[col] = 'GBS' # Default to GBS if not present
            elif col == 'bond_grade_type': df_copy[col] = 'Investment_Grade' # Updated default
            elif col == 'bond_sector': df_copy[col] = 'FIN' # Default sector
            elif col == 'bond_security_type': df_copy[col] = 'Secured'
            else:
    ...
    # Removed 'Age', 'Num_Bond_Investments_Made', 'Avg_Bond_Investment_Size', 'Education', 'Location', 'Gender'
    # as these are not in your final trained_feature_names.
    for col in ['bond_issued_date', 'bond_maturity_date', 'Transaction_Date', 'Synthetic_Customer_ID', 'bond_symbol_primary', 'bond_coupon_rate_str', 'bond_symbol_secondary']:
        if col not in df_copy.columns:
            if 'date' in col:
...
def preprocess_new_data(df_new_raw, encoder, trained_feature_names):
    ...
    # Based on your provided trained_feature_names, these are now explicitly defined:
    categorical_cols_to_ohe = [
        'bond_credit_rating', 'bond_sector', 'bond_security_type',
        'bond_grade_type', 'bond_issuer_type'
    ]

    # Ensure all categorical columns are actual categories for OHE.
    ...
    for col in categorical_cols_to_ohe:
        if col in df_for_ohe_and_numerical.columns:
            ...
            # These defaults should align with the 'Unknown' categories added to the global lists if applicable
            ...
            elif col == 'bond_grade_type': df_for_ohe_and_numerical[col] = 'Investment_Grade'
            elif col == 'bond_sector': df_for_ohe_and_numerical[col] = 'FIN'
            elif col == 'bond_security_type': df_for_ohe_and_numerical[col] = 'Secured'
            else: df_for_ohe_and_numerical[col] = 'Unknown' # Generic default
...
prediction_mode = st.sidebar.radio(
    ...
    help="Select 'Upload CSV File' to see the CSV upload option below."
)
...
if prediction_mode == "Manual Input":
    ...
    with col2:
        ...
        bond_credit_rating_input = st.selectbox("Bond Credit Rating", ALL_BOND_CREDIT_RATINGS, index=ALL_BOND_CREDIT_RATINGS.index('0')) # Default to '0'
        bond_issuer_type_input = st.selectbox("Bond Issuer Type", ALL_BOND_ISSUER_TYPES, index=ALL_BOND_ISSUER_TYPES.index('GBS'))
        bond_grade_type_input = st.selectbox("Bond Grade Type", ALL_BOND_GRADE_TYPES, index=ALL_BOND_GRADE_TYPES.index('Investment_Grade'))
        bond_sector_input = st.selectbox("Bond Sector", ALL_BOND_SECTORS, index=ALL_BOND_SECTORS.index('FIN'))
        bond_security_type_input = st.selectbox("Bond Security Type", ALL_BOND_SECURITY_TYPES, index=ALL_BOND_SECURITY_TYPES.index('Secured'))
...
# --- CSV Upload Section ---
    ...
    - *(Optional, for internal function robustness, but dropped for model: `bond_issued_date`, `bond_maturity_date`, `Transaction_Date`, `Synthetic_Customer_ID`, `bond_symbol_primary`, `bond_coupon_rate_str`, `bond_symbol_secondary`)*""")
